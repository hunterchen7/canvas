...
function printHelp() {
  console.log([
    ...
    "  --repetitions NUMBER       Profiled pairs per scenario/kind (default: 5)",
...
async function createIsolatedContext(browser, baseUrl) {
...
async function runTarget({
  ...
  const { context, errors } = await createIsolatedContext(browser, baseUrl);
...
async function main() {
  ...
  const settings = {
    ...
    targetOrder: "Alternates baseline-first and candidate-first for every pair.",
    captureMode: "One isolated cpu, trace, or allocations capture per action.",
    sourceVerification:
